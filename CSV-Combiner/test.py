...
class test(unittest.TestCase):
    ...
    def test_ext(self):
        ...
        with self.assertRaises(Exception) as context:
            csv_combinerdd.combine(['./fixtures/clothing.csv','./fixtures/clothing.xls'])
        self.assertEqual('incorrect file extension', str(context.exception))
    
    def test_exist(self):
        """checks for file extension """
        with self.assertRaises(Exception) as context:
            csv_combinerdd.combine(['./fixtures/clothing.csv','./fixtures/clothings.csv'])
    ...
    def test_filename_column_added(self):
        ...
        output = StringIO()
        sys.stdout = output
        csv_combinerdd.combine(['./fixtures/accessories.csv', './fixtures/clothing.csv'])
